    ...
    <div className="w-full max-w-[984px] mx-auto pb-16">
        ...
        {filters.map(({ key, label }) => (
          <button
            ...
            style={{
              ...
              background: filter === key ? '#000' : '#0F0F660D',
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {filtered.map(vault => {
          ...
          const cardContent = (
            <>
              <span className="inline-block self-start text-[13px] font-semibold text-white px-3 py-1 mb-5" style={{ background: '#000000' }}>
                {vault.badge}
              </span>

              <h3 className="text-[22px] mb-2" style={{ fontWeight: 700, color: '#000000' }}>
                {t(`vaults.${vaultKey}.title`)}
              </h3>
              <p className="flex-1 text-[18px] leading-relaxed mb-8 vault-card-desc" style={{ color: 'rgb(118, 119, 122)', fontWeight: 500 }}>
              ...
              <div className="flex gap-6 mb-8">
                <div>
                  ...
                  <p className="text-[18px]" style={{ fontWeight: 700, color: '#000000' }}>{vault.apr}</p>
                </div>
                <div>
                  <p className="text-[13px] mb-1" style={{ color: 'rgb(118, 119, 122)' }}>{t('earn.protocol')}</p>
                  <div className="flex items-center gap-1.5">
                    <YieldIcon size={18} />
                    <p className="text-[18px]" style={{ fontWeight: 700, color: '#000000' }}>{vault.yieldSource}</p>
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <TokenIcon size={18} />
                    <p className="text-[18px]" style={{ fontWeight: 700, color: '#000000' }}>{vault.tokenName}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                <span
                  ...
                  style={{ borderRadius: '28px', fontSize: '16px', lineHeight: '24px', fontWeight: 700 }}
                >
